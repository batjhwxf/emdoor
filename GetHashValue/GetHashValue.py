...
def create_table():
    ...
    try:
        select_count_key_id = "select count(*) from [dbo].[ProductHash] where [KEY_ID]='%s'" %(key_id)
        ret=cur.execute(select_count_key_id)  
        n = cur.fetchone()
        if n[0]:
            insert_dt_cmd="update [dbo].[ProductHash] set [STATE] = '%s', [HASH] = '%s' where [KEY_ID]= '%s';" %(key_state, key_hash, key_id)
        else:
...
try: 
  ...
  print("GetHashValueConfig error")
  print_error(str(e))
