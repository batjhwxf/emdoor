...
def set_status_15(po_number):
    try:
        
        update_status = "update [dbo].[ProductKeyInfo] set [ProductKeyStateID] = 15 where [ProductKeyStateID]=3 and [OEMPONumber]='%s';" %(po_number)
...
def set_status_3(po_number):
    try:
        f = open("keyid.txt")             # 返回一个文件对象
        line = f.readline().strip()       # 调用文件的 readline()方法
        while line:
            print("key id:", line)
            update_status = "update [dbo].[ProductKeyInfo] set [ProductKeyStateID]=3 where [ProductKeyStateID]=15 and [OEMPONumber]='%s' and [ProductKeyID]='%s';" %(po_number, line)
            line = f.readline().strip()
            #cur.execute(update_status)
        f.close()
        #conn.commit()       
    except Exception, e:
        print("Update3 data error")
        print_error(str(e))
